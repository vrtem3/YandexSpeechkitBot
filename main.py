...
def reply_voice_message(message: telebot.types.Message):
    try:
        ...
        tg_link_file = 'https://api.telegram.org/file/bot{0}/{1}'.format(TOKEN, file_info.file_path)
        ...
        cursor.execute("INSERT INTO message(date, chat_id, username, link_tg_voice, speech_text) VALUES(?, ?, ?, ?, ?)", 
                    (datetime.now().strftime('%d %B %Y %H:%M:%S'), message.chat.id, message.from_user.username, tg_link_file, text))
        conn.commit()
